"""
train.py — training pipeline robusta per binarizzazione impronte

Requisiti:
 - src.preprocessing.binarize.dataset.FingerprintBinaryDataset
 - src.preprocessing.binarize.model.UNetFingerprint (o UNetSmall senza sigmoid)
 - config/config_binarize.yml con sezioni: training, paths, dataset
"""
...
# Import your dataset & model (adatta i path se necessario)
from src.preprocessing.binarize.dataset import FingerprintBinaryDataset
from src.preprocessing.binarize.model import UNetFingerprint
...
def save_batch_visuals(img_in: torch.Tensor, logits: torch.Tensor, gt: torch.Tensor,
    ...
    """
    Salva (input, pred, gt) per i primi n esempi del batch.
    img_in: [B,1,H,W] in [0,1]
    logits: [B,1,H,W] raw
    gt: [B,1,H,W] in {0,1}
    """
    ...
    for i in range(n):
        ...
        # concatenazione verticale (input / prob / pred_binary / gt)
        cat = torch.cat([inp, pr, bin_pr, tgt], dim=2)  # stacks along width? ensure shape -> (C,H,4W) or viceversa
        # But To keep a simple grid we create a 4-row image (C,4,H) by permuting dimensions:
        # torchvision.save_image expects (B,C,H,W) or (C,H,W). We'll save three images separately:
...
# ----------------------------
def train_one_epoch(model: nn.Module, loader: DataLoader, optimizer, criterion_bce, criterion_dice,
                    device, scaler, epoch:int, cfg, max_grad_norm: float = 1.0):
    model.train()
    running_loss = 0.0
    ...
    for img_in, img_gt in pbar:
        ...
        batch_metrics = compute_metrics(logits.detach(), img_gt_bin.detach(), thr=cfg["training"].get("threshold", 0.5))
...
def validate(model: nn.Module, loader: DataLoader, criterion_bce, criterion_dice, device, cfg, epoch:int,
             vis_dir: Path = None):
    model.eval()
    running_loss = 0.0
    running_metrics = {"f1":0.0,"iou":0.0,"dice":0.0,"acc":0.0}
...
def main(config_path: str = "config/config_binarize.yml"):
    ...
    # Model
    model = UNetFingerprint(in_channels=1, out_channels=1, base=cfg["model"].get("base_channels", 64))
    ...
    # Scheduler: ReduceLROnPlateau by default, optional CosineWarmRestarts
    ...
    for epoch in range(1, epochs + 1):
        ...
        if scheduler_mode_plateau:
            ...
            # Cosine scheduler requires step per epoch with epoch index - 1 + fraction
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), ckpt_dir / f"best_unet_fingerprint.pth")
            print("✓ Saved best model (val loss improved)")
            epochs_no_improve = 0
        else:
            epochs_no_improve += 1

        # Save periodic and last
