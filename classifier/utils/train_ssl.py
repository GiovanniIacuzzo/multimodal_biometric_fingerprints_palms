...
def train_ssl(
    ...
    for epoch in range(epochs):
        ...
        # Log embeddings (opzionale) ogni N epoch
        if (epoch + 1) % 5 == 0:
            with torch.no_grad():
                sample = next(iter(dataloader))
                sample_i, _ = sample if isinstance(sample, (tuple, list)) else (sample, None)
                emb = model.backbone(sample_i[:32].to(device)).cpu().numpy()
                print(f"[DEBUG] Sample embedding norm mean: {np.mean(np.linalg.norm(emb, axis=1)):.4f}")
